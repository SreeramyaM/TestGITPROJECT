...
public class VerifyLandingPageOfAddMoreWorkersLinkTest extends VDRInitiator {
	...
	public Object[][] vdrApplicationData() throws Exception {
		JSONArray ja = (JSONArray) new JSONParser()
				.parse(new FileReader(
						VerifyLandingPageOfAddMoreWorkersLinkTest.class
								.getResource(
										"/org/fwcms/cdc/vdr/vdrValidationData.json")
								.getFile()));
	...
	public void testLandingPageOfAddMoreWorkersLink(HashMap<String, String> data)
			throws Exception {
		WebDriver driver = getDriver();
		driver.get(cdcUrl);
		new LoginPage(getDriver()).initElements().loginToCDC(
				EmployerCredentialsProp.getVdrEmployerUserName().trim(),
				EmployerCredentialsProp.getVdrEmployerPassword().trim());
		...
		cm.waitForOverlayToHide();
		wsp.verifyPresenceOfAddWorkersOptions();
		...
		WorkerPointOfEntryPage wpoep = new WorkerPointOfEntryPage(driver)
				.initElements();
		wpoep.clickAddMoreWorkersLink();
		cm.waitForOverlayToHide();
		wsp.verifyPresenceOfAddWorkersOptions();
		wsp.clickContinueButton();
